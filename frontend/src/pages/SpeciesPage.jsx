...
const speciesInfo = {
  dogs: {
    species: 'dog',
    title: 'Dog Breeds',
    description: 'Explore comprehensive information about dog breeds with detailed profiles, care requirements, and temperament guides.',
    color: 'from-amber-500 to-orange-600'
  },
  cats: {
    species: 'cat',
    title: 'Cat Breeds',
    description: 'Discover cat breeds with complete information on personality, grooming needs, and ideal living conditions.',
...
const SpeciesPage = () => {
  ...
  const info = speciesInfo[species] || speciesInfo.dogs;
  const alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('');

  useEffect(() => {
    loadBreeds();
  }, [species, selectedLetter, searchTerm, currentPage]);

  const loadBreeds = async () => {
    setLoading(true);
    try {
      const filters = { species: info.species };
  ...
  const availableLetters = useMemo(() => {
    // This would ideally come from the API, but for now we'll show all letters
    return alphabet;
  }, []);
  ...
  const breadcrumbs = [
    ...
    { name: info.title, url: `/breeds/${species}` }
  ];
